...
func BuildConfigChangeDetails(oldCfg, newCfg *config.Config) []string {
	...
	if len(oldCfg.MetaKey) != len(newCfg.MetaKey) {
		...
		for i := range oldCfg.MetaKey {
			...
			if strings.TrimSpace(o.Prefix) != strings.TrimSpace(n.Prefix) {
				changes = append(changes, fmt.Sprintf("meta[%d].prefix: %s -> %s", i, strings.TrimSpace(o.Prefix), strings.TrimSpace(n.Prefix)))
			}
			if o.Priority != n.Priority {
				changes = append(changes, fmt.Sprintf("meta[%d].priority: %d -> %d", i, o.Priority, n.Priority))
			}
			if o.Websockets != n.Websockets {
				changes = append(changes, fmt.Sprintf("meta[%d].websockets: %t -> %t", i, o.Websockets, n.Websockets))
			}
			if !reflect.DeepEqual(o.DisableCooling, n.DisableCooling) {
				changes = append(changes, fmt.Sprintf("meta[%d].disable-cooling: %s -> %s", i, formatOptionalBool(o.DisableCooling), formatOptionalBool(n.DisableCooling)))
			}
			if !reflect.DeepEqual(o.RequestRetry, n.RequestRetry) {
				changes = append(changes, fmt.Sprintf("meta[%d].request-retry: %s -> %s", i, formatOptionalInt(o.RequestRetry), formatOptionalInt(n.RequestRetry)))
			}
			if strings.TrimSpace(o.APIKey) != strings.TrimSpace(n.APIKey) {
